...
class App:
    ...
    def register_new_user(self):
        ...
        if st.sidebar.button('Register'):
            ret, frame = self.capture.read()
            if ret:
                new_user_image_path = os.path.join(self.db_dir, f"{name}.png")  # Save as PNG
    ...
    def run(self):
        ...
        with col2:
            ...
            st.write("")
            st.write("")
            login_icon = Image.open(r'C:\Users\chama\OneDrive\Bureau\projetGmc\CustomerSegmentation\images\login.jpeg')
            st.image(login_icon, width=100)
            if st.button('Login', key='login'):
                self.login()

            register_icon = Image.open(r'C:\Users\chama\OneDrive\Bureau\projetGmc\CustomerSegmentation\images\register.jpeg')
